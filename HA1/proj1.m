...
tau_N3 = mean(power_outputs3(:).*weights3(:)); %Taking the mean for each month, ignoring zeros
V_N3 = mean(V3(:));
...
V32 = zeros(1, 1000); % Pre-allocating for wind speed samplesy
power_outputs32 = zeros(1, 1000); % Pre-allocating for power output samples
...
weights32 = zeros(1000,1000);
for i = 1:1000
    for j = 1:1000
        [maxy, maxx] = max(P(V1(i)).*P(V2(j))'.*f12(V1(i), V2(j));
    end
end
