...
V3d2 = zeros(2, 1000); % Pre-allocating for wind speed samples
power_outputs3d2 = zeros(2, 1000); % Pre-allocating for power output samples

successes = zeros(1,1000);
fails = ones(1,1000);
for i = 1:1000
    V3d2(:,i)=mvnrnd(mu, covar_mat);
    weights32(i) = f12(V3d2(1,i),V3d2(2,i))/mvnpdf([V3d2(1,i),V3d2(2,i)], mu, covar_mat);
...
var_3d2s = var(successes.*weights32);
P_3d2s = mean(successes.*weights32);

var_3d2f = var(fails.*weights32);
P_3d2f = mean(fails.*weights32);


interval_3d2s = [P_3d2s - conf95 * sqrt(var_3d2s/1000) ; P_3d2s + conf95 * sqrt(var_3d2s/1000)];

interval_3d2f = [P_3d2f - conf95 * sqrt(var_3d2f/1000) ; P_3d2f + conf95 * sqrt(var_3d2f/1000)];
